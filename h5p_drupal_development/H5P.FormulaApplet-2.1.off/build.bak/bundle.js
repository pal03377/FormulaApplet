...
(function () {
	...
	*/
	...
	let domLoadResolve;
	let domLoad = new Promise(resolve => domLoadResolve = resolve);
	...
	async function initTranslation() {
	  if (!translationIsInitiated) {
	    ...
	    $$1(function () {
	      // $('.btn').button()
	      $$1('#de').on('click', function () {
	        switchTo('de');
	      });
	      $$1('#en').on('click', function () {
	        switchTo('en');
	      });
	    });
	...
	async function initEditor() {
	  await domLoad;
	  var before = $$1('div#ed_before');
	  if (before.length == 0) {
	    var ed = $$1('.formula_applet#editor');
	    ed.before('<p id="mode_select">');
	    $$1('p#mode_select').append('  <h3><span class="tr en mw-headline" id="Mode">Mode</span><span class="tr de mw-headline" id="Mode">Modus</span></h3>');
	    $$1('p#mode_select').append('  <input type="radio" id="auto" name="select_mode" checked />');
	    ...
	    $$1('p#mode_select').append(label_lse);
	    $$1('p#mode_select').append('  <br/>');
	    $$1('p#mode_select').append('  <input type="radio" id="manu" name="select_mode" />');
	    ...
	    $$1('p#mode_select').append(label_manu);
	    ed.before('<p id="input_id">');
	    $$1('p#input_id').append('  <label class="tr de idfa" for="fa_name">Id des Formel-Applets (4 bis 20 Zeichen)</label><label class="tr en idfa" for="fa_name">Id of Formula Applet (4 to 20 characters)</label>');
	    $$1('p#input_id').append('  <input type="text" id="fa_name" name="fa_bla_name" required minlength="4" maxlength="20" size="10">');
	    $$1('p#input_id').append('  <button type="button" class="tr de mfxi problemeditor" id="random-id-d">Zufalls-ID</button><button type="button" class="tr en mfxi problemeditor" id="random-id-e">Random ID</button>');
	    ...
	    var prepend_uses = $$1('.prepend_uses#p_u');
	...
	function keyboardEvent0(cmd) {
	    if (cmd.toLowerCase() == 'shift') {
	        ...
	        $$1(".formula_applet").trigger('customKeyboardEvent', cmd);
	        // keyboardEvent(cmd);
	        // switch back
	...
	async function preparePage() {
	  await domLoad;
	  $$1("img.mod").remove();
	  ($$1('<button class="keyb_button">\u2328</button>')).insertAfter($$1(".formula_applet"));
	  $$1('button.keyb_button').on('mousedown', function () {
	    showVirtualKeyboard();
	    $$1("button.keyb_button").removeClass('selected');
	  });
	  mathQuillify();
	  initTranslation();

	  $$1('body').on('click', function () {
	    $$1(".formula_applet").removeClass('selected');
	    $$1("button.keyb_button").removeClass('selected');
	  });

	  $$1('body').on('keyup', function (ev) {
	    var key = ev.originalEvent.key;
	    if (key == 'Tab') {
	      var fa = $$1(ev.target).parents('.formula_applet');
	      // var id = $(fa).attr('id');
	      fa.click();
	    }
	  });
	...
	function keyboardEvent(cmd) {
	  ...
	  if (typeof mf !== 'undefined') {
	    ...
	    if (cmd.startsWith('#')) {
	      ...
	      cmd = cmd.substring(1);
	      if (cmd == 'Enter') {
	        editHandler(activeMathfieldIndex);
	      } else if (cmd == 'setUnit') {
	        setUnit();
	...
	function checkIfEquality(id, equ, dsList) {
	  ...
	  if (dif < precision) {
	    $$1('#' + id).removeClass('mod_wrong').addClass('mod_ok');
	  } else {
	    $$1('#' + id).removeClass('mod_ok').addClass('mod_wrong');
	  }
	}
	...
	async function mathQuillify() {
	  ...
	  initVirtualKeyboard();
	  $$1(".formula_applet:not(.mq-math-mode)").each(function () {
	  ...
	  $$1(".formula_applet").each(async (index, domElem) => {
	    let element = $$1(domElem);
	    var fApp = new FApp();
	    ...
	    element.click(ev => {
	      if (fApp.hasResultField) {
	        ...
	        $$1(".formula_applet").removeClass('selected');
	        $$1(".formula_applet").off('customKeyboardEvent');
	        element.addClass('selected');
	        element.on('customKeyboardEvent', function (evnt, cmd) {
	          keyboardEvent(cmd);
	        });
	        $$1("button.keyb_button").removeClass('selected');
	        if ($$1('#virtualKeyboard').css('display') == 'none') {
	    ...
	    if (isEditor) {
	      ...
	      var mqEditableField = $$1('#editor').find('.mq-editable-field')[0];
	      // adjust events
	      $$1('#set-input-d, #set-input-e').on('mousedown', ev => {
	        ev.preventDefault();
	        setInput();
	      });
	      $$1('#set-unit-d').on('mousedown', ev => {
	        ev.preventDefault();
	        setUnit();
	      });
	      $$1('#set-unit-e, #set-unit-e').on('mousedown', ev => {
	        ev.preventDefault();
	        setUnit();
	      });
	      $$1('#erase-unit-d, #erase-unit-e').on('mousedown', ev => {
	        ev.preventDefault();
	        eraseUnit();
	      });
	      $$1('#random-id-d, #random-id-e').on('mousedown', ev => {
	        ev.preventDefault();
	        var rId = makeid(8);
	      ...
	      $$1('input[type="radio"]').on('click', ev => {
	        resultMode = ev.target.id;
	        if (resultMode == 'auto') {
	          $$1('span.mq-class.inputfield').prop('contentEditable', 'false');
	          showEditorResults(editorEditHandler(editorMf.latex()));
	        }
	        if (resultMode == 'manu') {
	          $$1('span.mq-class.inputfield').prop('contentEditable', 'true');
	          showEditorResults(editorEditHandler(editorMf.latex()));
	        }
	      });
	      $$1('#random-id-d').mousedown();
	      // $('input[type="radio"]#manu').click();
	      $$1('input[type="radio"]#auto').click();
	    } else {
	      //******************
	      // *** no editor ***
	      try {
	  ...
	  ($$1('<img class="mod">')).insertAfter($$1(".formula_applet.mq-math-mode:not(.solution)"));
	}
	...
	function showEditorResults(parts) {
	  ...
	  $$1('#output-code-1').text(parts[1]);
	  $$1('#output-code-2').text(result);
	  var out = $$1('textarea#wiki-text');
	  if (out.length > 0) {
	    out.text(result);
	  }
	}
